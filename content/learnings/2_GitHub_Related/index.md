...
Yes/   \No
 /       \
v         v
[Step 4: Handle merge conflicts]     [Step 5: Push merged result]
  git status                           git push
    |
    v
  (for each conflicted file, e.g. src/app.js)
    ...
    replace the whole block with FINAL version
    save file
    git add src/app.js
    git commit -m "Resolve merge conflicts in app.js"
    |
    v
  [Conflicts all resolved]
    |
    v
  git push        # now main (local & remote) has merged changes
    |
    v
[Step 6 (optional): Clean up dev branch]
  git branch -d dev
